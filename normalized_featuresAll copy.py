...
def nalize_features(input_path, output_path):
    ...
    df['bbox_min_left_n'] = gaussian_normalize(
    ...
    df['bbox_max_right_n'] = gaussian_normalize(
    ...
    df['bbox_min_top_n'] = gaussian_normalize(
        directional_normalize(df['bbox_min_left'], svg_center_x), 
        directional_normalize(df['bbox_min_top'], svg_center_y), 
        0, 0, sigma
    )
    ...
    # 下边界归一化，下边界在中心下面时为负
    df['bbox_max_bottom_n'] = gaussian_normalize(
        directional_normalize(df['bbox_max_right'], svg_center_x), 
        directional_normalize(df['bbox_max_bottom'], svg_center_y), 
        0, 0, sigma
    )
