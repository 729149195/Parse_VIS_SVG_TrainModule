...
class FeaturePairDataset(Dataset):
    def __init__(self, data_dir, sigma=1.0):
        self.sample_pairs = []
        self.sigma = sigma  # 高斯核参数，可以根据需要调整
        self.load_data(data_dir)

    def bbox_distance(self, f1, f2):
        # 假设特征维度 >= 15，且第12、13、14、15维对应索引为11,12,13,14
        # 根据描述，这四个维度代表定界框的左、右、上、下边界
        # 示例逻辑：取这四个值的差的绝对值，并选取其中的最小值作为距离
        bbox_f1 = f1[11:15]
        bbox_f2 = f2[11:15]
    ...
    def select_pairs(self, pairs, probs, identifier):
        ...
        # 使用概率分布按随机方式选取样本对（不确定性）
        selected_indices = np.random.choice(len(pairs), size=num_samples, replace=False, p=probs)
            ...
            if filename.endswith('.json'):
                ...
                with open(filepath, 'r') as f:
                    ...
                    for group in groups:
                        ...
                        # 使用高斯核生成正样本概率分布
                        pos_probs = self.gaussian_prob(pos_distances)
                        ...
                        for f_pos in group_features:
                            for f_neg in neg_sample_space:
                                dist = self.bbox_distance(f_pos, f_neg)
                        ...
                        # 使用高斯核生成负样本概率分布
                        neg_probs = self.gaussian_prob(neg_distances)
