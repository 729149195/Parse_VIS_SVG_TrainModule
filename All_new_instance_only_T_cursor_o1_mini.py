...
class FeatureVectorDataset(Dataset):
        ...
        for filename in os.listdir(data_dir):
            if filename.endswith('.json'):
                ...
                with open(filepath, 'r') as f:
                    ...
                    # 处理组
                    for group in groups:
                        group_features = group[:-2]  # 排除末尾的两个标识
                        group_label_pos = group[-2]
                        group_label_neg = group[-1]
                        group_indices = [features.index(torch.tensor(feature, dtype=torch.float32)) for feature in group_features]
                        # 标记已分配的特征
    ...
    def prepare_sampling(self):
        ...
        for idx, label in enumerate(self.labels):
            if label.item() not in self.group_to_indices:
                self.group_to_indices[label.item()] = []
            self.group_to_indices[label.item()].append(idx)
        
        # 加载groups标识
        self.group_labels = {}
        data_dir = './DataProduce/UpdatedStepGroups'
        for filename in os.listdir(data_dir):
            if filename.endswith('.json'):
                filepath = os.path.join(data_dir, filename)
                with open(filepath, 'r') as f:
                    ...
                    for group in groups:
                        group_label_pos = group[-2]
                        group_label_neg = group[-1]
                        group_features = group[:-2]
                        group_indices = [self.all_features.index(torch.tensor(feature, dtype=torch.float32)) for feature in group_features]
                        self.group_labels[self.label_from_indices(group_indices)] = (group_label_pos, group_label_neg)

    def label_from_indices(self, indices):
        """
        根据 group_indices 返回对应的标签。
        假设所有 group_indices 具有相同的标签，取第一个索引的标签并转换为整数。
        """
        return self.labels[indices[0]].item()
    ...
    def get_positive_negative_pairs(self, label):
        pos_label, neg_label = self.group_labels[label.item()]
        pos_indices = self.group_to_indices[label.item()]
        pos_samples = [self.all_features[i] for i in pos_indices]
        # 计算正样本空间的距离并应用t分布
        pos_distances = torch.cdist(torch.stack(pos_samples), torch.stack(pos_samples))
        pos_probs = torch.exp(-pos_distances)  # 简化t分布概率
        pos_probs = pos_probs / pos_probs.sum(dim=1, keepdim=True)
        if pos_label == 3:
            selected_pos = pos_indices
        elif pos_label == 2:
            selected_count = int(0.75 * len(pos_indices))
            selected_pos = torch.multinomial(pos_probs.sum(dim=0), selected_count).tolist()
        elif pos_label == 1:
            selected_count = int(0.5 * len(pos_indices))
            selected_pos = torch.multinomial(pos_probs.sum(dim=0), selected_count).tolist()
        else:
            selected_pos = pos_indices  # 默认全选
        # 类似地处理负样本空间
        neg_indices = list(set(range(len(self.all_features))) - set(pos_indices))
        neg_samples = [self.all_features[i] for i in neg_indices]
        neg_distances = torch.cdist(torch.stack(pos_samples), torch.stack(neg_samples))
        neg_probs = torch.exp(-neg_distances)  # 简化t分布概率
        neg_probs = neg_probs / neg_probs.sum(dim=1, keepdim=True)
        if neg_label == 3:
            selected_neg = neg_indices
        elif neg_label == 2:
            selected_count = int(0.75 * len(neg_indices))
            selected_neg = torch.multinomial(neg_probs.sum(dim=0), selected_count).tolist()
        elif neg_label == 1:
            selected_count = int(0.5 * len(neg_indices))
            selected_neg = torch.multinomial(neg_probs.sum(dim=0), selected_count).tolist()
        else:
            selected_neg = neg_indices  # 默认全选
